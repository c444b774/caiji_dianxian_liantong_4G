...
public class EricssonCdtCsvPublicParser extends FileParser implements ImsiQueryHelper.ASynchronizedParser {

	private static Logger LOGGER = LoggerFactory.getLogger(EricssonCdtCsvPublicParser.class);

	private static final Logger badWriter = BadWriter.getInstance().getBadWriter();
	 ...
	 */
	private final SimpleDateFormat cdrTimeFormat = new SimpleDateFormat("yyyyMMdd'T'HH:mm:ss");
	private final SimpleDateFormat cdrTimeNotTFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	...
	private static final String[] covertNumFields = {"CDR_MMEGROUPID","CDR_MMECODE","HO_FIRST_SRCCELLID","HO_FIRST_TGTCELLID","HO_LAST_TGTCELLID","RLS_CELLID","HO_LAST_SRCCELLID",
		"HO_HISTORYCELLECGI_1","HO_HISTORYCELLECGI_2","HO_HISTORYCELLECGI_3","HO_HISTORYCELLECGI_4","HO_HISTORYCELLECGI_5","HO_HISTORYCELLECGI_6",
		"HO_HISTORYCELLECGI_7","HO_HISTORYCELLECGI_8","HO_FIRST_TGTTAC"};
...
private static List<String> sourceFields = null;
	...
	public EricssonCdtCsvPublicParser(){
	...
	private ParseOutRecord extractNextReord() throws TimeoutException, ParseException {
		...
		boolean isreturnNull = AssociateCellUtil.associateCellInfo(task,recordData,nMinOrientNeiCellsNumber,Vendor.VENDOR_ERICSSON);
		//接入与释放都关联不上city_id，在切换与最后切换时再关联一次
		if(isreturnNull && StringUtils.isEmpty(recordData.get("CITY_ID"))){
			AssociateCellUtil.associateCityId(recordData,Vendor.VENDOR_ERICSSON);
		}
		if(!isreturnNull){
		...
		{
			cdrTime = cdrTimeNotTFormat.parse(cdrStartTime);
		}
...
//			badWriter.warn("非法文件时间记录.CDR_STARTTIME{}, record:{}", cdrStartTime, this.lineRecord);
		...
		// 厂家标识
		recordData.put("VENDOR", Vendor.VENDOR_ERICSSON);
		//ERAB_TOTAL_UPDATA,ERAB_TOTAL_DOWNDATA其它厂家值都是int，ericsson的带小数，经与产品沟通，对小数进行四舍五入取整
		String totalupdata = recordData.get("ERAB_TOTAL_UPDATA");
		String totaldowndata = recordData.get("ERAB_TOTAL_DOWNDATA");
		if(!StringUtils.isBlank(totalupdata))
			recordData.put("ERAB_TOTAL_UPDATA",String.valueOf(Math.round(Float.valueOf(totalupdata))));
		if(!StringUtils.isBlank(totaldowndata))
			recordData.put("ERAB_TOTAL_DOWNDATA",String.valueOf(Math.round(Float.valueOf(totaldowndata))));
	...
	/**
	 * 从文件名获取时间
	 * 	package: FD-LTE_CDR_ERICSSON_jlctcdrwk1_28135_20161226161500.csv.gz
	 * 	item:	FD-LTE_CDR_ERICSSON_jlctcdrwk1_28135_20161226161500.csv
	 * @param entryFileName
	 * @return
	 */
