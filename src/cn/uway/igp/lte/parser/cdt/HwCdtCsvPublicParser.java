...
public class HwCdtCsvPublicParser extends FileParser implements ImsiQueryHelper.ASynchronizedParser {
	...
	private static Logger LOGGER = LoggerFactory.getLogger(HwCdtCsvPublicParser.class);
...
//			badWriter.warn("非法文件时间记录.CDR_STARTTIME{}, record:{}", cdrStartTime, this.lineRecord);
	...
	private void conversionProcess(Map<String, String> recordData) throws Exception {
		//dpf 20170306 处理原始数据中RLS.longitude/RLS.latitude 、AC.longitude/AC.latitude 前的字符，如：N37.571440
		recordData.put("RLS_LONGITUDE", StringUtil.cutLetterDigitFront(recordData.get("RLS_LONGITUDE"),2));
		recordData.put("RLS_LATITUDE", StringUtil.cutLetterDigitFront(recordData.get("RLS_LATITUDE"),2));
		recordData.put("AC_LONGITUDE", StringUtil.cutLetterDigitFront(recordData.get("AC_LONGITUDE"),2));
		recordData.put("AC_LATITUDE", StringUtil.cutLetterDigitFront(recordData.get("AC_LATITUDE"),2));
